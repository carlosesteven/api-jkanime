...
import cheerio from '/Users/david/Desktop/jkanime/resolver.js';
import ky from 'ky';
import qs from 'qs';
...
async function latestAnimeAdded() {
  ...
  const animeData = _.chain(animeElements).filter((_2, index) => index % 2 !== 0).map(parseAnimeElement$1).thru((promises) => Promise.all(promises)).value().then((response2) => _.filter(response2, (anime) => anime.slug !== null));
  return animeData;
}
      ...
      const dayData = {
        ...
        animes: $("div.app-layout div.box.semana").eq(daysOfWeek.indexOf(day)).find("div.cajas div.box").map((index, element) => {
          ...
          return {
            ...
            slug: _.replace($el.find("h3").parent().attr("href"), /\//g, "") ?? null,
